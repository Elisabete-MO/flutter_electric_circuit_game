...
class ClosedLoopData {
  ...
  ClosedLoopData(this.components, this.wires);
}

/// Serviço de simulação física e solução de circuitos.
/// Isola a lógica matemática de gravação da UI Thread, sendo 100% pura e thread-safe.
class CircuitSolverService {
  /// Executa a simulação de circuito para um determinado estado.
  static SandboxState calculateSimulation(SandboxState targetState) {
    ...
    for (final source in powerSources) {
      ...
      final List<ClosedLoopData> closedLoops = [];
      ...
      _traverseForState(
        ...
        onLoopClosed: (pathComponents, pathWires) {
          closedLoops.add(ClosedLoopData(List.from(pathComponents), List.from(pathWires)));
        },
      );

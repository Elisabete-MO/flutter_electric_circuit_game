...
import '../../models/first_bench_flow.dart';
...
/// Coordenador principal do fluxo do Primeiro Estande (4 Fases).
class FirstBenchFlowScreen extends ConsumerStatefulWidget {
  const FirstBenchFlowScreen({super.key});

  @override
  ConsumerState<FirstBenchFlowScreen> createState() => _FirstBenchFlowScreenState();
}

class _FirstBenchFlowScreenState extends ConsumerState<FirstBenchFlowScreen> {
  late FirstBenchFlowState _flowState;
  ...
  Future<void> _loadState() async {
    try {
      ...
      final rawJson = prefs.getString(FirstBenchFlowState.storageKey);
      if (rawJson != null && rawJson.isNotEmpty) {
        final decoded = jsonDecode(rawJson) as Map<String, dynamic>;
        _flowState = FirstBenchFlowState.fromJson(decoded);
      } else {
  ...
  Future<void> _saveState() async {
    try {
      ...
      await prefs.setString(FirstBenchFlowState.storageKey, rawJson);
    } catch (_) {}
  }
  ...
  void _onPhaseCompleted(int phaseId) async {
    ...
    if (phaseId == 4) {
      // Concluiu o primeiro estande por completo!
      ref.read(progressControllerProvider.notifier).markAsCompleted('primeiros_passos', stars: 3);
      ref.read(progressControllerProvider.notifier).markAsCompleted('first_bench', stars: 3);
          ...
          child: GlassContainer(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Você concluiu todas as 4 fases! O primeiro ponto da Maquete Coletiva da Feira de Ciências foi iluminado com sucesso.',
                  style: TextStyle(

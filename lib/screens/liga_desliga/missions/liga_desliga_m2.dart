...
class _LigaDesligaM2State extends State<LigaDesligaM2>
      ...
      builder: (context, _) {
        return Row(
          children: [
            ...
            // Painel Lateral (Objetivo, Stepper de Previsão & Botão de Validação)
            Expanded(
              flex: 3,
  ...
  int get _currentStepperIndex {
    if (_answerStateA == null) return 0;
    if (_answerStateB == null) return 1;
    return 2;
  }

  bool _isStepCompleted(int index) {
    if (index == 0) return _answerStateA != null;
    if (index == 1) return _answerStateB != null;
    if (index == 2) return _answerStateA != null && _answerStateB != null;
    return false;
  }
  ...
  Widget _buildInvestigationStepperCard() {
    return WorkbenchInvestigationStepperCard(
      title: 'Progresso da previsão',
      currentStepIndex: _currentStepperIndex,
      isStepCompleted: _isStepCompleted,
      steps: const [
        'Analisar Cartaz A (Chave Aberta)',
        'Analisar Cartaz B (Chave Fechada)',
        'Confirmar previsão e energizar',
      ],
    );
  }
}
